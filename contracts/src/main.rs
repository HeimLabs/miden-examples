...
async fn main() -> Result<(), ClientError> {
    ...
    let counter_code = fs::read_to_string(counter_path).expect("Failed to read counter.masm");
    ...
    let script_code = fs::read_to_string(script_path).expect("Failed to read counter_script.masm");
    ...
    let account = client.get_account(counter_contract.id()).await?;
    println!(
